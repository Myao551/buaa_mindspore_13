...
class BaseSolver(object):
    ...
    def resume(self, path):
        ...
        '''
        # for cuda:0 memory
        state = mindspore.load_checkpoint(path, map_location='cpu')#rxz
    ...
    def load_tuning_state(self, path,):
        ...
        """
        if 'http' in path:
            #state = torch.hub.load_state_dict_from_url(path, map_location='cpu') #找不到
            print("solver.py / _line 147")
        else:
            state = mindspore.load_checkpoint(path, map_location='cpu')#rxz
    ...
    @staticmethod
    def _matched_state(state: Dict[str, mindspore.Tensor], params: Dict[str, mindspore.Tensor]):#rxz
        missed_list = []
        unmatched_list = []
        matched_state = {}
