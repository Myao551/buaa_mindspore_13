import mindspore  #llf
import mindspore.nn as nn  #llf 
import mindspore.dataset.transforms as T  #zxp
import mindspore.dataset.vision as RT #zxp
#from torch.cuda.amp import autocast  
from mindspore import amp #zxp
from mindspore import context #zxp
import numpy as np 
...
import argparse
import src.misc.dist as dist   #llf
...
def main(args, ):
    ...
    if args.resume:
        checkpoint = mindspore.load_checkpoint(args.resume, map_location='cpu') #zxp
    ...
    model = Model()
    amp_level = "O1" #zxp
    model = amp.auto_mixed_precision(model, amp_level) #zxp
    im_pil = Image.open(args.im_file).convert('RGB')
    w, h = im_pil.size
    orig_size = mindspore.Tensor([w, h])[None] #zxp
    ...
    if args.sliced:
        ...
        for i, slice_img in enumerate(slices):
            slice_tensor = transforms(slice_img)[None]
            # with autocast():  # Use AMP for each slice
            output = model(slice_tensor, mindspore.Tensor([[slice_img.size[0], slice_img.size[1]]])) #zxp
            # torch.cuda.empty_cache() #zxp
...
if __name__ == '__main__':
    ...
    # parser.add_argument('-d', '--device', type=str, default='cpu') #zxp
